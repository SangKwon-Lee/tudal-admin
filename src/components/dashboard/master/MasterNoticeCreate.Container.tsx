...
const MasterNoticeCreateContainer: React.FC<
  IMasterNoticeCreateProps
> = (props) => {
  const { user } = useAuth();
  ...
  const getFeedtList = async () => {
    try {
      const { data, status } = await APIMaster.getFeeds('', masterId);
      if (status === 200 && data.length > 0) {
        setFeedList(data);
        if (mode !== 'edit') {
  ...
  const getHiddenReportList = async () => {
    try {
      const { data, status } = await APIHR.getMasterReport(masterId);
      if (status === 200 && data.length > 0) {
        setReportList(data);
        if (mode !== 'edit') {
          setTargetId(data[0].id);
  ...
  const handleCreateNotice = async () => {
    ...
    try {
      if (mode === 'edit') {
        ...
        const { status } = await APIMaster.postMasterNotice(newData);
        if (status === 200) {
